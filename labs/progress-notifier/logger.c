...
int infof(const char *format, ...){
    ...
    int done;
    if(level == 0){
        char * sufix = (char *)malloc(sizeof(char)*(strlen(format) + 8));
        strcpy(sufix,"INFO: ");
		if(std)
...
int warnf(const char *format, ...){
    ...
    int done;
    if(level < 2){
        char * sufix = (char *)malloc((strlen(format) + 12)*sizeof(char));
...
int errorf(const char *format, ...){
    ...
    int done;
    if(level < 3){
        char * sufix = (char *)malloc((strlen(format) + 9)*sizeof(char));
        strcpy(sufix,"ERROR: ");
		if(std)
        	done = vfprintf(stdout, strcat(strcat(sufix, format), "\n\0"), arg);
		else{
